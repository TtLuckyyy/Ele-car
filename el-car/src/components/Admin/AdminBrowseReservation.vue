...
>
    ...
    <el-card class="box-card" v-if="currentReservation">
        ...
        <el-button type="danger" @click="deleteReservation">删除订单</el-button>
...
export default {
  ...
  methods: {
    ...
    async deleteReservation() {
      try {
        // 调用后端接口删除
        const response = await axios.delete(`/home/CarOwner/deleteReservation/${this.currentReservation.resId}`);

        if (response.data.status==='success') {
             // 从 reservationList 和 filteredReservationList 中移除已删除的订单
            this.reservationList = this.reservationList.filter(
                reservation => reservation.resId !== this.currentReservation.resId
            );
            this.filteredReservationList = this.filteredReservationList.filter(
                reservation => reservation.resId !== this.currentReservation.resId
            );
    ...
    this.filteredReservationList = this.reservationList.filter((reservation) => {
        ...
        console.log(statusMatch, dateMatch);
