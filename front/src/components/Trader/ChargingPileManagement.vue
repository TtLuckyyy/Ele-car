...
export default {
  ...
  methods: {
    ...
    async fetchChargingPiles() {
      try {
        const response = await axios.get('/home/Trader/ChargingPilesList'); // 请求后端数据
        
        const data = response.data;
        
        // 获取当前用户的运营商品牌
        const currentTraderResponse = await this.$http.get(`/home/Trader/${this.user.id}`); // 请求后端数据

        this.chargingPiles = data.filter(pile => { return Number(pile.trader_id) === Number(currentTraderResponse.data.trader_id);});
        ...
        // 遍历数据，为每个充电桩对象随机分配图片
        this.chargingPiles = this.chargingPiles.map(pile => {//map方法遍历数组，执行回调函数，返回一个新的数组
          return {
            ...pile,
            imageUrl: imageUrls[Math.floor(Math.random() * imageUrls.length)], // 随机选择图片
          };
        });
      } catch (error) {
    ...
    async saveChanges() {
        ...
        if (response.data.status==='success') {
          ...
          if (index !== -1) {
            this.chargingPiles.splice(index, 1, { ...this.editForm }); // 用新的数据替换旧的数据,splice(index,num,newItem)表示替换数组元素，第一个参数是索引，第二个参数是删除的元素个数，第三个参数是插入的元素
            console.log('Updated Charging Piles:', this.chargingPiles); // 查看数据是否更新成功
          }
    ...
    changeImage() {
      // 模拟图片更改
      const newImageUrl = 'https://via.placeholder.com/300'; // 你可以根据需求修改图片上传功能
